...
static void     __scHandleSomething(OSSched *s, OSScTask *t);
...
OSScTask *gSchedulerCurrTask;
u8 gSchedulerNotGfxTask;
                       ...
                       u8 mode, u8 numFields)
{
    ...
    gSchedulerCurrTask = 0;
    gSchedulerNotGfxTask = 0;
    
    osCreateMesgQueue(&sc->interruptQ, sc->intBuf, OS_SC_MAX_MESGS);
    osCreateMesgQueue(&sc->cmdQ, sc->cmdMsgBuf, OS_SC_MAX_MESGS);
    osCreateMesgQueue(&sc->myQ, sc->myMsgBuf, OS_SC_MAX_MESGS);
...
OSMesgQueue *osScGetMyQ(OSSched *sc)
{
    return &sc->myQ;
}
    ...
    while (1) {
        ...
        switch ((int) msg) {
            ...
            */
            ...
            default:
                __scHandleSomething(sc, (OSScTask *) msg);
                break;
        }
    }
}

void __scHandleSomething(OSSched *sc, OSScTask *t)
{
    u32 pad[2];
    s32 state;
    u32 pad2;
    ...
    if (t != gSchedulerCurrTask) {
        gSchedulerCurrTask = t;
        __scAppendList(sc, t);
        state = ((sc->curRSPTask == 0) << 1) | (sc->curRDPTask == 0);
        if ((func_80007668 (sc, &sp, &dp, state) != state) && (gSchedulerNotGfxTask == 0))
            __scExec(sc, sp, dp);
    }
}
...
void __scHandleRetrace(OSSched *sc)
{
    ...
    if (sc->doAudio && sc->curRSPTask) {
        ...
        state = ((sc->curRSPTask == 0) << 1) | (sc->curRDPTask == 0);
        if ( func_80007668 (sc, &sp, &dp, state) != state)
            __scExec(sc, sp, dp);
    }
...
#else
#pragma GLOBAL_ASM("asm/nonmatchings/boot/scheduler/func_80007668.s")
#endif
