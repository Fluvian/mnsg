...
##### Flags #####
INCLUDES := -Iinclude -Iinclude/2.0I -Iinclude/2.0I/PR -Isrc -I.

MIPSISA := -mips2
OPTFLAGS := -O2

ASFLAGS := -EB -mtune=vr4300 -march=vr4300 -mabi=32 -Iinclude
CFLAGS := -G 0 -non_shared -Xfullwarn -Xcpluscomm $(INCLUDES) -Wab,-r4300_mul -woff 649,838,712 -D_LANGUAGE_C -D_FINALROM -DF3DEX_GBI
...
$(TARGET).elf: $(LD_SCRIPT) $(O_FILES)
	$(LD) -T $(LD_SCRIPT) -Map $(TARGET).map -T symbol_addrs.$(VERSION).txt -T undefined_syms.$(VERSION).txt -T undefined_syms_auto.txt -T undefined_funcs_auto.txt --no-check-sections -o $@
...
setup: baserom.$(VERSION).z64
	make -C tools
	python3 tools/splat/split.py $(BASENAME).$(VERSION).yaml
